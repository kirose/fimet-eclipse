...
public class MasterCardParser extends AbstractMessageParser {
	...
	protected void formatHeader(IWriter writer, Message msg) {
		...
		writer.append(new ConverterBuilder(msg.getMti().getBytes(), ConverterBuilder.TYPE_ASCII).toEbcdic().toBytes());
	}
	@Override
	...
	protected void parseHeader(IReader reader, Message msg) {
		String mti = reader.ebcdic(4).toAscii().toString();
		msg.setHeader("");
		msg.setMti(mti);
	}
	@Override
