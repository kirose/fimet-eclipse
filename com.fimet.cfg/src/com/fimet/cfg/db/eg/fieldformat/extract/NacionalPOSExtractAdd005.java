...
public class NacionalPOSExtractAdd005 {
	FieldFormatDAO dao;
	int idGroup = FieldFormatGroup.EXTRACT_ADD005_POS.getId();
	public NacionalPOSExtractAdd005(FieldFormatDAO dao) {
		this.dao = dao;
	}
	public void insert() {
		...
		dao.insertOrUpdate(new FieldFormat(idGroup,"008","8",NONE.getId(),null,null,"(?s).+",2,null,"DATASET ID",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"009","9",NONE.getId(),null,null,"(?s).+",9,null,"CONVENIO CIE",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"010","10",NONE.getId(),null,null,"(?s).+",20,null,"REFERENCIA",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"011","11",NONE.getId(),null,null,"(?s).+",7,null,"GUIA CIE",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"012","12",NONE.getId(),null,null,"(?s).+",40,null,"RAZON SOCIAL",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"013","13",NONE.getId(),null,null,"(?s).+",12,null,"FILLER",FixedFieldParser.class));
		dao.insertOrUpdate(new FieldFormat(idGroup,"014","14",NONE.getId(),null,null,"(?s).+",16,null,"PAN TARJETA CORRESPONSAL",FixedFieldParser.class));
	}
}
