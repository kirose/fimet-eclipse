...
public class MIN extends Function {
	public MIN(ParserFormula parserFormula){
		super(parserFormula);
	}
	public MIN(ParserFormula parserFormula, Expression ... args){
		super(parserFormula,args);
	}
	public Expression parse(Token token, ParserFormula parser, int end) {
		args = parser.parseArgs(end);
		if (args == null || args.length < 2){
			throw new ParserFormulaException("Sintaxis invalida se esperaban almenos 2 argumrntos MIN (arg1, arg2[, ...])");
		}
		return this;
	}
	...
	public TypeDouble eval(TypeDouble v0, TypeDouble v1){
		return new TypeDouble(Math.min(v0.getValue(), v1.getValue()));
	}
	public TypeInteger eval(TypeInteger v0, TypeInteger v1){
		return new TypeInteger(Math.min(v0.getValue(), v1.getValue()));
	}
	...
	public String toString(){
		return "least("+ ParserUtils.implode(args, ", ") + ")";
	}
}
