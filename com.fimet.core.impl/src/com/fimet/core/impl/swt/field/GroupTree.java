...
public class GroupTree extends TreeViewer {
    ...
    public TreeItem getTreeItem(GroupNode v) {
    	TreeItem[] items = getTree().getItems();
    	for (TreeItem i : items) {
			if (i.getData() == v) {
				return i;
			} else {
				return findTreeItem(i, v);
			}
		}
    	return null;
    }
    public TreeItem findTreeItem(TreeItem parent, GroupNode node) {
    	return null;
    }
	...
	public void update(GroupNode parent, FieldFormatGroup group) {
		GroupNode node = parent != null ? parent.getNode(group) : null;
		refresh();
		getTree().deselectAll();
		TreeItem item = getTreeItem(node);
		if (item != null)
