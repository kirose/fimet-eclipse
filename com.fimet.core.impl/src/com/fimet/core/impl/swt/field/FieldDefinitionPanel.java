...
public class FieldDefinitionPanel extends Composite implements IFieldController {
    ...
    protected final Control createContents(Composite container, int stylePanel) {
    	...
    	treeGroups.getTree().addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				loadFieldsIntoTable(treeGroups.getSelectedNode());
			}
			@Override
    ...
    private void loadFieldsIntoTable(GroupNode group) {
		if (group != null && !group.equals(treeFields.getGroup())) {
			List<FieldFormat> fields = fieldFormatGroupManager.getFieldsFormat(group.getGroup().getId());
			if (!fields.isEmpty() && !historyFields.isEmpty(group.getGroup().getId())) {
			...
			treeFields.setGroup(group);
		}    	
    }
	public void onNewGroup() {
		GroupDialog dialog = new GroupDialog(getShell(), SWT.NONE);
		dialog.open();
		FieldFormatGroup sm = dialog.getFieldFormatGroup();
		if (sm != null) {
			treeGroups.add(null, sm);
			historyGroups.insert(this.getSelectedGroup().getId(), sm);
			loadFieldsIntoTable(treeGroups.getSelectedNode());
		}
	}
	public void onAddGroup() {
    	if (treeGroups.getSelectedNode() == null) {
			...
			FieldFormatGroup sm = dialog.getFieldFormatGroup();
			if (sm != null) {
				treeGroups.add(treeGroups.getSelectedParent(), sm);
				if (sm.getId() == null) {
					historyGroups.insert(this.getSelectedGroup().getId(), sm);
				} else {
					historyGroups.update(this.getSelectedGroup().getId(), sm);
				}
			}
    	}
	}
    ...
    public void onNewField() {
		FieldDialog dialog = new FieldDialog(this, getShell(), SWT.NONE);
		dialog.open();
		FieldFormat sm = dialog.getFieldFormat();
		if (sm != null) {
			treeFields.add(null, sm);
			historyFields.insert(this.getSelectedGroup().getId(), sm);
		}
	}
    public void onAddField() {
    	if (treeFields.getSelectedNode() == null) {
    		onNewField();
    	} else {
			FieldDialog dialog = new FieldDialog(this, treeFields.getSelectedNode(), null, getShell(), SWT.NONE);
			dialog.open();
			FieldFormat sm = dialog.getFieldFormat();
			if (sm != null) {
				FieldNode parent = treeFields.getSelectedNode();
				treeFields.add(treeFields.getSelectedNode(), sm);
				historyFields.update(this.getSelectedGroup().getId(), parent.getField());
				if (sm.getIdFieldFormat() == null) {
					historyFields.insert(this.getSelectedGroup().getId(), sm);
				} else {
					historyFields.update(this.getSelectedGroup().getId(), sm);
				}
			}
    	}
	}
    public void onEditField() {
		if (treeFields.getSelected() != null) {
			FieldDialog dialog = new FieldDialog(this, treeFields.getSelectedParentNode(), treeFields.getSelected(), getShell(), SWT.NONE);
			dialog.open();
			FieldFormat sm = dialog.getFieldFormat();
			if (sm != null) {
				if (sm.getIdGroup() != getSelectedGroup().getId()) {
					sm.setIdGroup(getSelectedGroup().getId());
					sm.setIdFieldFormat(null);
				}
				treeFields.update(treeFields.getSelectedParentNode(),  sm);
				historyFields.update(this.getSelectedGroup().getId(), sm);
			}
		}
	}
