package com.fimet.editor.json.preferences.format;
...
import com.fimet.editor.json.core.BundleUtils;
...
import com.fimet.editor.json.preferences.format.JsonFormatter.Affix;
...
public class JsonFormatPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private void createTextViewer(Composite appearanceComposite, int numColumns) {
        ...
        textViewer.getTextWidget().setFont(JFaceResources.getFont(Activator.getInstance().FONT_ID));
        textViewer.setEditable(false);

        try {
            String text = BundleUtils.readFile(Activator.getInstance().getBundle(), "/formatter_example.json");
            textViewer.setDocument(new Document(text));
        } catch (IOException e) {
            JsonLog.error("Could not load example json file", e);
        }
