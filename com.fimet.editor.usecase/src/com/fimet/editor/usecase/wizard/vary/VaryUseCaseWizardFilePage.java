...
public class VaryUseCaseWizardFilePage extends WizardPage implements Listener {
	...
	private VaryUseCaseWizard wizard;
	//private VText txtMsg;
	
	public VaryUseCaseWizardFilePage(String pageName, VaryUseCaseWizard wizard) {
		super(pageName);
	...
	public void createControl(Composite parent) {
	    ...
	    configureFileName(wizard.getUseCaseResource());
	}


	private void configureFileName(IResource useCaseSrc) {
		if (useCaseSrc == null) {
	...
	public class ResourceAndContainerGroupCopy extends ResourceAndContainerGroup {
		...
		public ResourceAndContainerGroupCopy(Composite parent, Listener client, String resourceFieldLabel, String resourceType, boolean showClosedProjects, int heightHint,IResource resource) {
			super(parent, client, resourceFieldLabel, resourceType, showClosedProjects, heightHint,resource);
		}
		...
		public boolean validateControls() {
			...
			VaryUseCaseWizardFilePage.this.validateControls();
			return isValid;
		}
	}
	 ...
	 *         not created
	 */
	public IFile createNewFile(IResource useCaseRes, String nameFile, List<Parameter> params) {
		...
		if (resource != null && new java.io.File(resource.getLocation().toString()).exists()) {
			try {
				ResourcesPlugin.getWorkspace().delete(new IResource[] {resource}, true, new NullProgressMonitor());
			} catch (CoreException e) {}
		}
		UseCase useCase = Manager.get(IUseCaseManager.class).parse(useCaseRes);
		if (useCase == null) {
			return null;
		}
		replaceFields(useCase, params);
	...
	private void replaceFields(UseCase useCase, List<Parameter> params) {
		Message msg = useCase.getAcquirer().getRequest().getMessage();
		for (Parameter p : params) {
			msg.setValue(p.getKey(), p.getValue());
		}
	}
