...
public class ResourceAndContainerGroup implements Listener {
	...
	private IResource useCaseResources;
	...
	public ResourceAndContainerGroup(Composite parent, Listener client,
			...
			boolean showClosedProjects, int heightHint, IResource useCaseResources) {
		super();
	...
	protected boolean validateFullResourcePath(IPath resourcePath, String resource) {
		...
		if (useCaseResources.equals(res)) {
			problemType = PROBLEM_RESOURCE_EXIST;
			problemMessage = NLS.bind(IDEWorkbenchMessages.ResourceGroup_nameExists, resource);
			return false;
		}
	...
	protected boolean validateResourcesName(List<String> resources) {
		...
		if (resources!= null && resources.size() == 0) {
			...
			problemMessage = NLS.bind("The number of file names {0} must be >= 1.",resources.size());
			return false;
		}
		return true;
	}
