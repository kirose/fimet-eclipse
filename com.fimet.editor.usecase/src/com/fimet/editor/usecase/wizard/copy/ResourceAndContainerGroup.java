...
public class ResourceAndContainerGroup implements Listener {
	 ...
	 * Constant for path already occupied.
	 */
	public static final int PROBLEM_NAME_COUNT_NOT_MATCHES = 9;
	...
	public ResourceAndContainerGroup(Composite parent, Listener client,
			...
			boolean showClosedProjects, int heightHint, List<IResource> useCaseResources) {
		super();
		this.useCaseResources = useCaseResources;
	...
	protected void createContents(Composite parent, String resourceLabelString,
		...
		resourceNameField = new Text(nameGroup, SWT.BORDER | SWT.V_SCROLL | SWT.MULTI);
	...
	public List<String> getResources() {
		if (resourceNameField.getText().length() == 0) {
			return null;
		}
		String resourcesString = resourceNameField.getText();
		String[] filesNames = resourcesString.indexOf("\r\n") != -1 ? resourcesString.split("\\r\\n") : resourcesString.split("\\n");
		List<String> resources = new ArrayList<>();
		for (String fileName : filesNames) {
			resources.add(useResourceExtension(fileName) ? (fileName + '.' + resourceExtension) : fileName);
		}
		return resources;
	}
	 ...
	 * @since 3.3
	 */
	private boolean useResourceExtension(String resource) {
	...
	protected boolean validateControls() {
		...
		List<String> resources = getResources();
		if (!validateContainer() || !validateResourcesName(resources)) {
			return false;
		}
		boolean valid = true;
		IPath path;
		for (String resource : resources) {
			path = containerGroup.getContainerFullPath().append(resource);
			valid &= validateFullResourcePath(path, resource);
		}
		return valid;
	}
	 ...
	 */
	protected boolean validateFullResourcePath(IPath resourcePath, String resource) {
		IWorkspace workspace = ResourcesPlugin.getWorkspace();
		...
		if (!allowExistingResources
			...
			problemMessage = NLS.bind(IDEWorkbenchMessages.ResourceGroup_nameExists, resource);
			return false;
		}
		IResource res = workspace.getRoot().findMember(resourcePath);
		if (useCaseResources.contains(res)) {
			//problemType = PROBLEM_RESOURCE_EXIST;
			problemMessage = "Will be oberride "+resource;
			return true;
		}
		return true;
	}
	 ...
	 */
	protected boolean validateResourcesName(List<String> resources) {
		...
		for (String resource : resources) {
			if (!Path.ROOT.isValidPath(resource)) {
				...
				problemMessage = NLS.bind(IDEWorkbenchMessages.ResourceGroup_invalidFilename,resource);
				return false;
			}	
		}
		if (resources!= null && resources.size() != useCaseResources.size()) {
			problemType = PROBLEM_NAME_COUNT_NOT_MATCHES;
			problemMessage = NLS.bind("The number of file names must be equals to {0}.",useCaseResources.size());
			return false;
		}
		return true;
	}
