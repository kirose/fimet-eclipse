...
public class NewUseCaseWizardPage extends WizardPage implements Listener {
	...
	// widgets
	private ResourceAndContainerGroupTrn resourceGroup;
	...
	private AcquirerCombo cvIapAcquirer;
	private IssuerCombo cvIapIssuer;
	private NewUseCaseWizard wizard;
	
	public NewUseCaseWizardPage(String pageName, NewUseCaseWizard wizard) {
		super(pageName);
	...
	protected void createIapAcquirerControls(Composite parent, int nColumns) {
        ...
        cvIapAcquirer = new AcquirerCombo(group);
        cvIapAcquirer.getCombo().setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 3, 1));
        cvIapAcquirer.getCombo().addSelectionListener(new SelectionAdapter() {
        	public void widgetSelected(SelectionEvent e) {
        		if (cvIapAcquirer.getStructuredSelection() != null) {
        			cvIapAcquirer.valid();
        			ISocket acquirer = (ISocket)cvIapAcquirer.getStructuredSelection().getFirstElement();
        			...
        			if (useCase.getAcquirer().getRequest().getMessage() != null) {
	        			...
	        			if (useCase.getAcquirer().getRequest().getMessage().getMti() != null) {
	        				useCase.getAcquirer().getRequest().getMessage().setMti(NewUseCaseWizardMsgPage.DEFAULT_MTI);
	        			}
        			}
        			//String template = MessageTemplates.getTemplate(useCase.getAcquirer().getRequest().getMessage().getMti(),useCase.getAcquirer().getIap().getParser().toString());
        			wizard.getSecondPage().setMsgTemplate("");
        		}
        	}
        });
	...
	protected void createIapIssuerControls(Composite parent, int nColumns) {
		...
		cvIapIssuer = new IssuerCombo(group);
		cvIapIssuer.getCombo().setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 3, 1));
        cvIapIssuer.getCombo().addSelectionListener(new SelectionAdapter() {
        	public void widgetSelected(SelectionEvent e) {
        		if (cvIapIssuer.getStructuredSelection() != null) {
        			cvIapIssuer.valid();
        			if (useCase.getIssuer() == null) {
        				useCase.setIssuer(new Issuer());
        			}
        			useCase.getIssuer().setConnection((ISocket)cvIapIssuer.getStructuredSelection().getFirstElement());
        		}
        	}
        });
	...
	protected void createTransactionNameControls(Composite composite, int nColumns) {
		...
		resourceGroup = new ResourceAndContainerGroupTrn(
			topLevel,
			this,
			getNewFileLabel(),
	...
	public boolean isPageComplete() {
		return (resourceGroup.isValid() && cvIapAcquirer.isValid() && cvIapIssuer.isValid());
	}
	@Override
	...
	public class ResourceAndContainerGroupTrn extends ResourceAndContainerGroup {
		private boolean isValid;
		public ResourceAndContainerGroupTrn(Composite parent, Listener client, String resourceFieldLabel, String resourceType, boolean showClosedProjects, int heightHint) {
			super(parent, client, resourceFieldLabel, resourceType, showClosedProjects, heightHint);
		}
		...
		public boolean validateControls() {
			...
			NewUseCaseWizardPage.this.validateControls();
			return isValid;
		}
	}
	...
	protected InputStream getInitialContents() {
		if (useCase.getAcquirer().getRequest().getMessage() == null) {
			useCase.getAcquirer().getRequest().setMessage(new Message());
		}
		useCase.getAcquirer().getRequest().getMessage().setMti(NewUseCaseWizardMsgPage.DEFAULT_MTI);
		//if (useCase.getAcquirer().getRequest().getMessage().getFields().isEmpty()) {
			//ISourceConnection iap = (ISourceConnection)cvIapAcquirer.getStructuredSelection().getFirstElement();
			//if (iap != null) {
				//MessageTemplates.createTemplate(useCase.getAcquirer().getRequest().getMessage(), iap.getParser().toString());
			//}
		//}
		return new ByteArrayInputStream(useCase.toJson().getBytes());
	}
	...
	public ISocket getAcquirer() {
		return cvIapAcquirer.getSelected();
	}
	public ISocket getIssuer() {
		return cvIapIssuer.getSelected();
	}
}
